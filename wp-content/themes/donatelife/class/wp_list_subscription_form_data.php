<?php 
class Subscription_Form_List extends WP_List_Table {
    ...
    function get_columns() {
        $columns = [
            ...
            'id'       => __('ID', 'donatelife'),
            's_name'       => __('Name', 'donatelife'),
            's_email'     => __('Email', 'donatelife'),
            's_mobile'    => __('Phone', 'donatelife'),
            's_date'     => __('Date', 'donatelife'),
    ...
    public function process_bulk_action() {
        global $wpdb;
        $table_name = $wpdb->prefix . 'subscription_master';
    ...
    private function table_data() {
        ...
        if (!empty($_REQUEST['s'])) {
            ...
            $where = " WHERE s_name LIKE '%$search%' OR s_email LIKE '%$search%' OR s_mobile LIKE '%$search%'";
        }
        $table_name = $wpdb->prefix . 'subscription_master'; // Replace with your custom table name
        $results = $wpdb->get_results("SELECT * FROM $table_name $where ORDER BY id DESC", ARRAY_A); //is_trash = 0
