<?php 
class Donor_Recepients_List extends WP_List_Table {
    ...
    function get_columns() {
        ...
        $table_name = $wpdb->prefix . 'finalmastersheet'; // Replace with your actual table name

        // Retrieve all column names from the table
        $columns_data = $wpdb->get_results("SHOW COLUMNS FROM $table_name", ARRAY_A);
        ...
        // Loop through each column and add it to the columns array
        foreach ($columns_data as $column) {
            if($column['Field'] !='finalmastersheetDate' && $column['Field'] != 'username' && $column['Field'] != 'createdate' && $column['Field'] != 'modifieddate' && $column['Field'] != 'sortorder' && $column['Field'] != 'status' && $column['Field'] != 'remote_ip' && $column['Field'] != 'is_trash') {
                
                $columns[$column['Field']] = __(ucwords(str_replace('_', ' ', $column['Field'])), 'donatelife');
            }
        }
    ...
    function column_default($item, $column_name) {
        ...
        if (array_key_exists($column_name, $item)) {
            return $item[$column_name]; // Return the value for the specified column
        }
    
        // Handle specific columns for actions and view
    ...
    public function process_bulk_action() {
        global $wpdb;
        $table_name = $wpdb->prefix . 'finalmastersheet';
    ...
    private function table_data() {
        ...
        //     $where = " where latestFigureYear LIKE '%$search%' OR noofCadaver LIKE '%$search%' OR kidneyDonation LIKE '%$search%' OR kidneyTransplanted LIKE '%$search%' OR liverDonation LIKE '%$search%' OR liverTransplanted LIKE '%$search%' OR pancreasesTransplant LIKE '%$search%' OR heartTransplant LIKE '%$search%' OR cornea LIKE '%$search%' OR bones LIKE '%$search%'";
        // }
        $table_name = $wpdb->prefix . 'finalmastersheet'; 
        ...
        $results = $wpdb->get_results("SELECT * FROM $table_name $where ORDER BY id ASC", ARRAY_A); //WHERE is_trash = 0 
